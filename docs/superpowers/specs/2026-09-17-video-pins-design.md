...
- New module `src/render/renderVideo.ts`:
  ...
  - Cost — **measured 2026-09-17, both v1 formats**: 51–79 s per video on the laptop (337 frames for the checklist reveal, 349 for the countdown, ~150–230 ms each). The original "300 frames × ~40 ms ≈ 15 s" underestimated the per-frame screenshot by 4–6×; a full-page capture at 1.08× is the cost, not the animation stepping. Still acceptable at one list/day (~7 min for a week's batch).
...
- **Variants:** keep `VARIANTS_PER_LIST = 4` and the test that ties it to `TEMPLATE_NAMES.length`. Video is *not* a fifth static variant; it is a separate `VIDEO_TEMPLATES` map with its own `clarity video` stage so the static queue math and the daily cadence stay untouched.
- **How many videos per list — decided 2026-09-22: one, drawn from a pool of two templates** (`checklist-reveal`, `numbers-countdown`), picked by the list-name hash so consecutive lists alternate. Not two per list, and not one per video template. The 5/day budget (`PINS_PER_DAY`) is fixed, so each video *displaces a static* rather than adding to the day — and statics earn 4.8 outbound clicks/1k against video's 1.6. At 5 pins/day: 4 statics + 1 video = 5 pins/list = **1.0 lists/day and 1.0 videos/day**, which already sits exactly on the "never more than 1 video/day" ceiling; a second video per list would mean 1.67 videos/day and break it. A third template waits until a board actually needs one (sticky-note peel for the seasonal boards). Revisit at day 30 — and if video clears the 2× saves gate, the move is **3 statics + 2 videos**, keeping 5 pins/list so throughput stays flat, not 6 pins/list.
...
- **Packs:** `src/stages/pack.ts` writes one extra pack per list with `video: <template>.mp4`, `cover: <template>-video-cover.png`, same title/description/alt/board/link as the statics (alt text describes the *cover*). Slot rule: the video pack goes through `assignDates` like any other pack, so it lands **`URL_GAP_DAYS` (3) after the last static variant** — day 12 of the list's run, not day 10.

  **Corrected 2026-09-22.** The original rule here ("the day after the last static variant, so it rides the chain-reaction window") contradicted this spec's own §"Pinterest video facts", which says spacing applies *regardless of format*, and `src/schedule.ts` would have refused the date anyway: `assignDates` skips any day where the same `destUrl` is within `URL_GAP_DAYS`, and every pin for a list shares one blog-post URL. The chain-reaction argument does not survive the rule that produced it. The video is simply the 5th item in the list's queue and takes the next legal slot.
...
- **Cadence:** week 1–2, **2 videos/week** (Tue/Fri) — a hand-picked subset of lists, not every list — then one per list from week 3 if the numbers hold. Never more than 1 video/day: video is reach, statics are clicks, and the fresh-pins/day ceiling counts both formats. **Note the budget moved:** `PINS_PER_DAY` went 3 → **5** on 2026-09-17 to work through the four-variant backlog, so "one video per list" now lands exactly on 1 video/day. If the cadence drops back to 3/day, one video per list falls to 0.6 videos/day and there is headroom again.
