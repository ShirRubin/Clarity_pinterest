...
npm test                   # run the node:test suites (schedule + approve + queue + revise + destination + status + schema + packText + posted + topics + postplan + reconcile + rowForPack + ship + notionPage + decide + review-notion + review-access + review-handler + publishedFlip + migrateScheduled)
...
- **`Scheduled` vs `Published`**: `clarity posted` sets `Scheduled` when all four variants are in Pinterest's scheduler; `Published` means the first variant's date has passed (flipped by the nightly job on the first local day after the scheduled date — `src/publishedFlip.ts`). `pack` (ex-`publish`) never changes status.
