...
- `src/stages/*.ts` — one module per stage (Phase 1+: ideas/draft use the Claude API; load the `claude-api` skill before writing API code).
...
`NOTION_TOKEN`, `NOTION_PARENT_PAGE_ID`, `NOTION_DB_ID` (set), `PINTEREST_APP_ID=1603049` (Trial pending), `ANTHROPIC_API_KEY` (needed for Phase 1), `PINTEREST_APP_SECRET`/`ACCESS_TOKEN` (Phase 4).
